...
class Bot:
    ...
    def start(self):
        ...
        logger.info(f'{self.__class__.__name__} is up and listening to new messages for you.........)
    ...
    def download_user_photo(self, quality=0):
        ...
        if self.current_msg.content_type != 'photo':
            raise RuntimeError(f"type 'photo' expected, "
                     f"but got {self.current_msg['content_type']}")
...
class YoutubeBot(Bot):
    def handle_message(self, message):
        if self.current_msg.content_type == 'photo':
            ...
            try:
                self.send_text('Hello! , thanks for your message! '
